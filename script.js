...
function handleSignup(){
  ...
  if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){ err.textContent = 'Please enter a valid email address (e.g. name@example.com).'; return; }
...
function appendFlashcard(front, back){
  ...
  card.innerHTML = `
    <div class="flashcard-inner">
      ...
      <div class="flashcard-face flashcard-back">${escapeHtml(back)}</div>
    </div>`;
  grid.appendChild(card);
}

function bumpCardCount(n){
  const made = parseInt(localStorage.getItem('snapstudy_cardcount')||'0') + n;
  localStorage.setItem('snapstudy_cardcount', made);
  refreshDashStats();
}

function renderFlashcardsFromBank(subject){
  const cards = FLASHCARD_BANK[subject] || FLASHCARD_BANK.maths;
  document.getElementById('flash-grid').innerHTML = '';
  cards.forEach(([front, back])=>appendFlashcard(front, back));
  bumpCardCount(cards.length);
}

function renderFlashcardsFromPhotoText(lines){
  document.getElementById('flash-grid').innerHTML = '';
  lines.forEach((line, i)=>{
    const colonIdx = line.indexOf(':');
    let front, back;
    if(colonIdx > 2 && colonIdx < line.length - 2){
      front = line.slice(0, colonIdx).trim();
      back = line.slice(colonIdx+1).trim();
    }else{
      front = 'From your notes #' + (i+1);
      back = line;
    }
    appendFlashcard(front, back);
  });
  bumpCardCount(lines.length);
}

/* Actually reads the uploaded photo using in-browser OCR (Tesseract.js) — no fake results.
   If no readable text is found (e.g. a photo of a person, not notes), it says so honestly
   instead of silently showing unrelated subject cards. */
async function generateFlashcards(){
  const subject = document.getElementById('flash-subject').value;
  const previewImg = document.getElementById('preview-img');
  const hasImage = !previewImg.classList.contains('hidden') && previewImg.src;
  const grid = document.getElementById('flash-grid');

  if(!hasImage){
    renderFlashcardsFromBank(subject);
    toast('No photo uploaded — showing quick revision cards for ' + subject + '.');
    return;
  }

  grid.innerHTML = '<div class="card" style="text-align:center;color:var(--muted);">🔍 Reading the text in your photo — this can take a few seconds...</div>';

  try{
    if(typeof Tesseract === 'undefined') throw new Error('OCR engine not loaded');
    const { data: { text } } = await Tesseract.recognize(previewImg.src, 'eng');
    const lines = text
      .split('\n')
    ...
    if(lines.length >= 2){
      renderFlashcardsFromPhotoText(lines.slice(0, 6));
      toast('Flashcards generated from your photo!');
    }else{
      grid.innerHTML = '';
      renderFlashcardsFromBank(subject);
      toast("Couldn't find readable notes text in that photo — here are quick " + subject + " cards instead.");
    }
  }catch(err){
    grid.innerHTML = '';
    renderFlashcardsFromBank(subject);
    toast("Couldn't scan that photo (needs an internet connection) — showing " + subject + " cards instead.");
  }
}
